...
def create_pause_model(task, n_features, uncertainty):
    ...
    if task == 'classification':
        model.add(layers.Dense(2, activation='softmax', kernel_regularizer=tf.keras.regularizers.l2(0.01),
...
def create_compare_model(task, features_size, uncertainty):
    ...
    model.add(layers.Dense(24, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01),
                           activity_regularizer=tf.keras.regularizers.l1(0.01)))
    model.add(layers.BatchNormalization())
    model.add(layers.Dropout(0.2))
    ...
    if task == 'classification':
        ...
        if uncertainty:
            ...
            model.add(layers.Dense(8, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                   activity_regularizer=tf.keras.regularizers.l1(0.01)))
            model.add(layers.Dense(8, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                   activity_regularizer=tf.keras.regularizers.l1(0.01)))
...
def create_silences_model(task, uncertainty):
    ...
    if task == 'classification':
        ...
        if uncertainty:
            ...
            model2_output = layers.Dense(1, kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l1(0.01))(model2_output)
            model2_output = layers.ReLU(max_value=30)(model2_output)
