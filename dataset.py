...
dataset_dir=r'C:\Users\HP\Downloads\datset_dir\train'
...
# def prepare_data(dataset_dir, config):
    ...
    #def prepare_data(dataset_dir, config):
        ...
        #for file_path in subject_files:
            #split_path = re.split('[/-]', file_path)
            #subject_id = split_path[-2]  # Assuming the subject ID is the second-to-last element
            #subject_ids.append(subject_id)

        #subjects = np.array(sorted(list(set(subject_ids))))
...
def prepare_data(dataset_dir, config):
    ...
    # print([re.split(r'\\', i)[-1][:-4] for i in subject_files])
    # AA = [[-2] for i in subject_files]
    # print(AA)
    subject_ids = []
    # for file_path in subject_files:
    #     print(file_path,"FILEPATHH")
    #     split_path = re.split('[/-]', file_path)
    #     subject_id = split_path[-2]  # Assuming the subject ID is the second-to-last element
    #     subject_ids.append(subject_id)

    # subjects = np.array(sorted(list(set(subject_ids))))
    subjects = np.array(sorted(list(set([re.split(r'[\\/-]', i)[-1][:-4] for i in subject_files]))))
    # Call the function
    #print(subjects,"SSSS")

    # Call the function
    dataset_dir = r'path/to/dataset_dir'
    config = types.SimpleNamespace()  # Update with your desired config
    prepare_data(dataset_dir, config)
    ######################################################################
    ...
    #print(cc_transcription_files,"cc_transcription_files")
    if dataset_dir == '../DementiaBank':
        #print(dataset_dir,"DATAASET")
    ...
    assert np.array_equal(y_intervention, y_pause) and X_intervention.shape[0] == X_pause.shape[
        0], '~ Data streams are different ~'
...
def prepare_data_new(dataset_dir, config):
    ...
    '''
    ################################## SUBJECTS ################################
    subject_files = sorted(glob.glob(os.path.join(dataset_dir, 'Full_wave_enhanced_audio/cc/*.wav')) + glob.glob(
        os.path.join(dataset_dir, 'Full_wave_enhanced_audio/cd/*.wav')))

    #print(subject_files,"subject_files")
    subjects = np.array(sorted(list(set([i.split(r'\\')[-1][:-4] for i in subject_files]))))
    print(subjects)
    ...
    if 'ADReSS' in dataset_dir:
        ...
        cc_audio_files = sorted(glob.glob(os.path.join(dataset_dir, 'Full_wave_enhanced_audio/cc/*.wav')))
    #print(cc_audio_files,"going to print")

    #The reason for using a maximum length of 800 could be a design choice
    #or a requirement specific to the application or dataset being used.
    #This value is used to clip the length of the pause features extracted from the audio files.
    #Pause features in audio files refer to characteristics or indicators of pauses or silences within the audio signal.
    #Pauses or silences are periods of time in the audio where there is little or no sound or speech activity.
    max_len = 800
    ...
    #import numpy as np
    #all_counts_cc, all_counts_cd, and X_pause
    #all_counts_cc has 2 rows and represents the pause features of CC audio files.
    #all_counts_cd has 4 rows and represents the pause features of CD audio files.
    #X_pause has 4 rows and represents the combined pause features of both CC and CD audio files.
